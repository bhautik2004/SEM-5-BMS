...
if (isset($_POST['submit'])) {
    ...
    $res = mysqli_query($con, "INSERT INTO account_open (name , gender , mobile , email , birthdate , pan_number , home_address , office_address , state , city , pincode , account_no , ifsc , branch , nominee_name , nominee_account_number , nominee_account_type , acc_open_date , account_status ) VALUES('$name' , '$gender' , '$mobile' , '$email' , '$birthdate' , '$pan_number' , '$home_address' , '$office_address' , '$state' , '$city' , '$pincode' , '$account_no' , '$ifsc' , '$branch' , '$nominee_name' , '$nominee_account' , '$nominee_account_type' , '$acc_open_date' , 'pending' )");
    if ($res) {
        echo '<script>alert("Application submitted successfully\n\Account number : ' . $account_no . '\n\nPlease visit bank with this Account number for account approval\n Within 15 Days")
...
<body>
    ...
    <form method="post" action="<?php $_SERVER['PHP_SELF'];?>" onsubmit="return validateForm()">
        ...
        <input type="email" name="email" id="email" class="b" placeholder="Email">
         <input placeholder="Date Of Birth" class="b" name="birthdate" type="text" onfocus="(this.type='date')" id="birthdate" />
